...
fn get_recent_blockhash(client: &RpcClient) -> (Hash, FeeCalculator) {
    loop {
        match client.get_recent_blockhash_with_commitment(CommitmentConfig::processed()) {
            Ok(Response {
                value: (blockhash, fee_calculator, _last_valid_slot),
                ..
...
pub fn do_bench_tps(
    ...
    let blockhash = Arc::new(RwLock::new(get_recent_blockhash(client.as_ref()).0));
    ...
    loop {
        ...
        if let Some(txs0) = txs {
            ...
            for tx in txs0 {
                ...
                if !client.send_transaction(&tx.0) {
...
pub fn airdrop_lamports(
    ...
    if starting_balance < desired_balance {
        ...
        let _signature = client
            .request_airdrop_with_blockhash(&id.pubkey(), airdrop_amount, &blockhash)
            .unwrap_or_else(|err| {
                panic!(
                    "Error requesting airdrop: to addr: {:?} amount: {} {:?}",
                    faucet_addr, airdrop_amount, err
                )
            });
