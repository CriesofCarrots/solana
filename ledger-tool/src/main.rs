...
fn main() {
    ...
    let matches = App::new(crate_name!())
        ...
        .subcommand(
            SubCommand::with_name("destroy-roots")
                .about("Traverses the AncestorIterator backward from a last known root \
                        to restore missing roots to the Root column")
                .arg(
                    Arg::with_name("start_root")
    ...
    match matches.subcommand() {
        ...
        ("repair-roots", Some(arg_matches)) => {
            ...
            let roots_to_fix: Vec<_> = ancestor_iterator
                .filter(|slot| {
                    let missing = !blockstore.is_root(*slot);
                    println!("{:?} missing: {:?}", slot, missing);
                    missing
                })
                .collect();
            if roots_to_fix.len() > 0 {
                blockstore
                    .set_roots(&roots_to_fix)
                    .unwrap_or_else(|err| {
                        eprintln!("Unable to set roots {:?}: {}", roots_to_fix, err);
                        exit(1);
                    });
            } else {
                println!("No missing roots found in range {} to {}", end_root, start_root);
            }
        }
        ("destroy-roots", Some(arg_matches)) => {
            let blockstore = open_blockstore(
                &ledger_path,
                AccessType::TryPrimaryThenSecondary,
                wal_recovery_mode,
            ...
            assert!(start_root > end_root);
            assert!(blockstore.is_root(start_root));
            let roots: Vec<_> = (end_root..start_root).collect();
            blockstore.destroy_roots(&roots).unwrap();
        }
